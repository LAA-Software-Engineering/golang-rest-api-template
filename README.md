...
On first boot against an empty database, the schema in [`pkg/database/schema.sql`](./pkg/database/schema.sql) is created automatically, so local bootstrap needs no manual step. This is **not** a migration system — see [Database schema and evolving it](#database-schema-and-evolving-it) for how to change the schema of an existing database. See [Getting Started](#getting-started) for the full walkthrough.
...
### Database schema and evolving it

The database schema lives in [`pkg/database/schema.sql`](./pkg/database/schema.sql). At startup `ApplySchema` runs it with `CREATE TABLE IF NOT EXISTS`, which **bootstraps an empty database only**. It is deliberately *not* a migration tool: Postgres no-ops those statements once a table exists, so editing `schema.sql` and redeploying against a database that already has the table will **not** add new columns, indexes, or constraints. (This is a known trade-off versus GORM's `AutoMigrate`, which did apply additive changes.)

To change the schema of a database that already has data, you must apply the change yourself — run the `ALTER TABLE ...` by hand, or adopt a migration tool ([golang-migrate](https://github.com/golang-migrate/migrate), [goose](https://github.com/pressly/goose), or [atlas](https://atlasgo.io)) and keep `schema.sql` as the source of truth sqlc reads. Keep `schema.sql` in sync with the live schema either way, since sqlc generates code against it.

The type-safe query code in [`pkg/repository/dbsqlc/`](./pkg/repository/dbsqlc) is generated by [sqlc](https://sqlc.dev) from `schema.sql` plus the queries in [`pkg/repository/queries/`](./pkg/repository/queries) (config in [`sqlc.yaml`](./sqlc.yaml)). After changing the schema or any query, regenerate and commit the output:
